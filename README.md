...
Canonical ISO 3166-2 subdivisions are available in politically named files under `data/country/`, such as `AD.parish.json`, `US.state_outlying_area_district.json`, or `FR.subdivision.json` when ISO defines many subdivision types for one country. These files use ISO subdivision suffixes as keys, include the full ISO code in `iso3166-2`, preserve exact ISO names/types in `iso-name` and `iso-type`, and include `parent-code` where ISO defines a parent subdivision. Each country file's `data.division-hierarchy` describes the political subdivision hierarchy and points to the relevant subdivision file(s). Regenerate them with `scripts/add-iso3166-2-subdivisions.py` and validate them with `scripts/validate-iso3166-2-subdivisions.py`.
